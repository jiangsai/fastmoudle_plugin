 ...
 *
 *      properties 全名称应该是 <plugin_name>.properties, 如{@link com.xincheng.android.gradle.plugin.maven.MavenPlugin}
 *                 对应的属性文件是maven.properties;
 *                 可以是project级的properties,也可以是rootProject级的properties,
 *                 如果同时存在,project级properties优先级更高
 *
 *      plugin.properties 所有xkit-plugin插件的属性都可以定义在这里
 *
 ...
 *          or
 *      Property.load("xxx.properties").propertyResult("key").matches(100) {*          // do something
 *}*      ...
 *
 * 在gradle中通过 {@link com.xincheng.android.gradle.plugin.api.XExtension} 使用:
 *      def value = xExt.property("key")
 *
 */
class Property {
