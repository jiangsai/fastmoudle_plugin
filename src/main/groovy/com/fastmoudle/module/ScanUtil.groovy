...
class ScanUtil {
    ...
    static final PROXY_CLASS_PREFIX = "Fast\$\$"
    static final PROXY_CLASS_SUFFIX = "\$\$Proxy.class"
    static final PROXY_CLASS_PACKAGE_NAME = "com/fastmoudle/android/generated/module"
    static final REGISTER_CLASS_FILE_NAME = "com/fastmoudle/fastmoudle_interfaces/module/ModuleLoader.class"

    //包含生命周期管理初始化类的文件，及包含 com.fastmoudle.fastmoudle_interfaces.module.ModuleLoader 类的class文件或者jar文件
    ...
    static List<String> scanJar(File jarFile, File destFile) {
        ...
        while (enumeration.hasMoreElements()) {
            ...
            System.out.println("scanJar=====>"+entryName)
            if (entryName == REGISTER_CLASS_FILE_NAME) {
                //标记这个jar包包含 ModuleLoader.class
                ...
                if (entryName.startsWith(PROXY_CLASS_PACKAGE_NAME)) {
                    ...
                    Logger.logd("scan jar : " + jarFile.getAbsolutePath())
                    Logger.logd("scan dest jar : " + destFile.getAbsolutePath())
